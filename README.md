...
Per l'iterazione il programma utilizza un ciclo ```cpp while(<condizione>) {<istruzione> | <blocco di istruzioni>}```
```cpp
while (i < n) {
    // istruzioni
    i = i + 1;
...
```
che può essere sostituito con un ciclo ```cpp for(<inizializzazione>; <condizione>; <espressione_iterativa>) {<istruzione> | <blocco di istruzioni>}```
```cpp
for (i=0; i<n; i++) {
    // istruzioni
}
